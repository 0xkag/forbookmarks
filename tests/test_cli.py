...
class TestCli(unittest.TestCase):
    def setUp(self):
        ...
        os.makedirs(
            os.path.dirname(self.test_html_file), exist_ok=True
        )  # Ensure dir exists
    ...
    def test_cli_html_exec_env_vars(self):
        ...
        result = self.runner.invoke(
            ...
            [
                ...
                "html",
                "-x",
                f"{self.test_script_path}",
            ],
        )
        self.assertEqual(
            result.exit_code,
            0,
            msg=f"CLI Error: {result.output} {result.exception} {result.exc_info}",
        )
    ...
    def test_cli_html_exec_args(self):
        ...
        self.assertIn(
            "ARGS=prefix_arg http://cli.example.com/test1 CLI Test 1", result.output
        )
    ...
    def test_cli_missing_file(self):
        ...
        self.assertIn(
            "Error: Invalid value for '--input-file' / '-f': Path 'tests/nosuchfile.html' does not exist.",
            result.output,
        )
