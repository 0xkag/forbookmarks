...
class TestJsonParser(unittest.TestCase):
    def setUp(self):
        ...
        firefox_sample_content = {
            "title": "Bookmarks Menu", "root": "placesRoot", "children": [
                {"title": "Mozilla Firefox", "uri": "https://www.mozilla.org/firefox/", "type": "text/x-moz-place"},
                {"title": "Work Stuff", "type": "text/x-moz-place-container", "children": [
                    {"title": "Internal Wiki", "uri": "http://internal.example.com/wiki", "type": "text/x-moz-place"}
                ]}
            ]
        }
    ...
    def test_parse_chrome_json(self):
        ...
        expected_bms = [
            ...
            {'url': 'https://arstechnica.com', 'title': 'Ars Technica', 'path': ['Bookmarks bar', 'Tech Blogs']}
        ]
    ...
    def test_parse_firefox_json(self):
        ...
        self.assertEqual(len(bookmarks), 2)
        expected_bms = [
            {'url': 'https://www.mozilla.org/firefox/', 'title': 'Mozilla Firefox', 'path': ['Bookmarks Menu']},
            {'url': 'http://internal.example.com/wiki', 'title': 'Internal Wiki', 'path': ['Bookmarks Menu', 'Work Stuff']}
        ]
        for bm in expected_bms:
            self.assertIn(bm, bookmarks)
