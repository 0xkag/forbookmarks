...
@click.option(
    "--exec",
    "-x",
    "exec_command",  # Renamed from exec_command_str
    type=str,
    required=True,
...
def process(  # pylint: disable=too-many-arguments, too-many-locals
    input_file: str,
    input_format: str,
    exec_command: str,  # Renamed from exec_command_str
    env_vars: bool,
    args: bool,
    path_separator: str,
) -> None:
    ...
    for bookmark in bookmark_iterator:  # Renamed from bookmark_item
        url: str = str(bookmark.get("url", ""))
        title: str = str(bookmark.get("title", ""))
        path_list: List[str] = [str(p) for p in bookmark.get("path", []) if p]
        bookmark_path: str = path_separator.join(path_list)  # Renamed from path_str

        current_env: Dict[str, str] = os.environ.copy()
        if env_vars:
            current_env["BOOKMARK_URL"] = url
            current_env["BOOKMARK_TITLE"] = title
            current_env["BOOKMARK_PATH_STR"] = bookmark_path
            for i, part in enumerate(path_list):
                current_env[f"BOOKMARK_PATH_{i}"] = part

        # C0301: Line too long (221/100) - Reformatting
        final_command: str = exec_command.replace("{URL}", url)  # Renamed
        final_command = final_command.replace("{TITLE}", title)
        final_command = final_command.replace("{PATH}", bookmark_path)

        # Using shlex.split for robust command parsing is better,
        # but for now, simple split.
        command_parts: List[str] = final_command.split()  # Renamed from cmd_list

        if args:
            command_parts.append(url)
            command_parts.append(title)
            command_parts.extend(path_list)
        ...
        click.echo(f"  Path: {bookmark_path}")

        try:
            # pylint: disable=subprocess-run-check
            process_result: subprocess.CompletedProcess[str] = subprocess.run(
                command_parts, env=current_env, capture_output=True, text=True
            )
            ...
            click.echo(
                f"Error executing '{' '.join(command_parts)}': {exception}", err=True
            )
